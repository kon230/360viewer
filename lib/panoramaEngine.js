...
// Same glyph as the floor-plan camera pins (components/FloorPlan.js's
// CameraGlyph) so a floor marker in the panorama and its destination pin
// on the floor plan read as the same visual language.
const CAMERA_ICON_SVG = `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M4 8.5C4 7.67 4.67 7 5.5 7h2.1l.9-1.5A1.5 1.5 0 0 1 9.79 4.7h4.42c.53 0 1.02.28 1.29.75L16.4 7h2.1c.83 0 1.5.67 1.5 1.5v8c0 .83-.67 1.5-1.5 1.5h-13A1.5 1.5 0 0 1 4 16.5v-8Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/>
  <circle cx="12" cy="12.5" r="3.1" stroke="currentColor" stroke-width="1.6"/>
</svg>`;
...
export class PanoramaEngine {
  ...
  // Used while a camera/plan has no image assigned yet, instead of a
  // synthetic placeholder graphic.
  setBlackTexture() {
    const gl = this.gl;
    gl.bindTexture(gl.TEXTURE_2D, this.tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([0, 0, 0, 255]));
  ...
  setMarkers(markers) {
    ...
    for (const m of this.markers) {
      ...
      if (!el) {
        ...
        label.className = 'pano-marker-label';
        el.appendChild(icon);
      ...
      // Only the dedicated icon/label children are rewritten here, never el
      // itself: callers (e.g. React portals) may attach extra children (a
      // popover) directly to el, and those must survive repeated calls.
      el.dataset.kind = m.kind || 'nav';
      const icon = el.querySelector('.pano-marker-icon');
      icon.innerHTML = m.kind === 'info' ? `<span class="pano-marker-dot"></span>` : CAMERA_ICON_SVG;
      const label = el.querySelector('.pano-marker-label');
      label.textContent = m.label || '';
      label.style.display = m.label ? '' : 'none';
  ...
  _attachInputHandlers() {
    ...
    window.addEventListener('pointermove', (e) => {
      if (this._draggedMarkerId) {
        ...
        // Repositioning is an edit-mode-only action; outside edit mode a
        // marker only supports click-to-navigate, so dragging must be a
        // no-op rather than silently relocating the marker.
        if (this.editMode) {
          const yp = this._screenToYawPitch(e);
